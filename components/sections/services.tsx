...
export function Services() {
  ...
  return (
    <section id="servicios" className="noise relative overflow-hidden bg-[#0a0b0a] py-20 text-white sm:py-28 lg:py-36">
      <div className="absolute -left-32 bottom-0 size-[32rem] rounded-full bg-violet-600/15 blur-[130px]" />
      <div className="absolute -right-32 top-20 size-[30rem] rounded-full bg-[#b8ff3d]/8 blur-[130px]" />
        ...
        <Reveal>
          <div className="flex flex-col justify-between gap-8 border-b border-white/10 pb-10 lg:flex-row lg:items-end">
            ...
            <div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[.16em] text-white/30">
              <MousePointer2 className="size-3.5 text-[#b8ff3d]" /> Explora el sistema
            </div>
          </div>
        </Reveal>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-[.82fr_1.18fr]">
              ...
              {services.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "group flex min-h-16 items-center gap-3 rounded-2xl border px-3 py-2 text-left outline-none transition duration-300 focus-visible:ring-2 focus-visible:ring-[#b8ff3d]",
                      isActive ? "border-[#b8ff3d] bg-[#b8ff3d] text-black" : "border-white/10 bg-white/[.03] text-white/50 hover:border-white/25 hover:text-white",
                    )}
                  >
                    <span className={cn("grid size-10 shrink-0 place-items-center rounded-xl transition", isActive ? "bg-black text-[#b8ff3d]" : "bg-white/5 text-white/45 group-hover:text-[#b8ff3d]")}>
          ...
          <Reveal delay={.08}>
            <div className="relative flex min-h-[38rem] h-full overflow-hidden rounded-[2rem] border border-white/12 bg-[#131513] p-6 sm:p-9">
              ...
              <div className="absolute right-[-8%] top-[-8%] size-72 rounded-full border border-[#b8ff3d]/20" />
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="flex items-start justify-between">
                    <span className="font-mono text-[9px] uppercase tracking-[.18em] text-[#b8ff3d]">Capability / 0{active + 1}</span>
                    <motion.div initial={reduceMotion ? false : { rotate: -18, scale: .8 }} animate={{ rotate: 0, scale: 1 }} className="grid size-20 place-items-center rounded-full border border-[#b8ff3d]/30 bg-[#b8ff3d]/10 text-[#b8ff3d] shadow-[0_0_50px_rgba(184,255,61,.1)] sm:size-24">
                  ...
                  <div className="mt-auto max-w-xl pt-20">
                      ...
                      {phases.map((phase, index) => (
                        <div key={phase} className="bg-[#131513] p-3">
                          <div className="mb-3 flex items-center justify-between">
                            ...
                            <Check className="size-3 text-[#b8ff3d]" />
                          </div>
                    ...
                    <a href="#contacto" className="mt-8 inline-flex items-center gap-3 rounded-full bg-white px-5 py-3 text-xs font-semibold text-black transition hover:-translate-y-0.5 hover:bg-[#b8ff3d]">
        ...
        <Reveal>
          <div className="mt-5 flex flex-col items-center justify-between gap-5 rounded-[1.5rem] border border-white/10 bg-white/[.035] p-5 sm:flex-row sm:px-7">
            <div className="flex items-center gap-4">
              <span className="grid size-11 place-items-center rounded-full bg-violet-500/15 text-violet-300"><Layers3 className="size-5" /></span>
              <div>
            ...
            <a href="#contacto" className="inline-flex shrink-0 items-center gap-2 text-xs font-semibold text-[#b8ff3d] transition hover:gap-3">Evaluar mi proyecto <ArrowDownRight className="size-3.5" /></a>
          </div>
        </Reveal>
      </div>
    </section>
